---
description: Батчит задачи из Trello — группирует лёгкие задачи в батчи для совместной работы и проверяет наличие инструкции как фиксить. Вызывается только оркестратором @task-manager, напрямую пользователем не используется.
mode: subagent
hidden: true
temperature: 0.2
permission:
