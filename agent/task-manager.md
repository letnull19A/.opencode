---
description: Управляет Trello-задачами с тегом проекта через scripts/task-manager/*. Оркестрирует только сабагента аудита (task-audit). Не трогает Trello API сам — только скрипты, никогда не пишет код и не вызывает кодовых агентов (build/refactor/unit-test). Используй когда просят завести, переместить задачу, тикет, карточку в Trello или проверить статус задач — не когда просят написать код.
mode: all
temperature: 0.2
permission:
  edit: deny
