...
## Классификатор (обязательно перед делегированием)
...
- **Код — не твоя зона:** никогда не вызываешь сабагентов для кода (`build`, `build-fast`, `build-smart`, `refactor`, `unit-test`, `component-builder`, `evol-plan`, `diagnostics`, `router`). Единственные разрешённые сабагенты — `task-audit` (аудит), `task-batch` (батчи), `task-explain` (объясни задачу) — и только через `classify_task`. Просьба «сделай фичу X / почини Y» — это приказ **создать карточку** `Добавить X` / `Починить Y` в Trello по строгому формату (`## Контекст`/`## Что сделать`/`## Критерии`), а не писать код и не делегировать кодовым агентам. Кодом занимаются только `@build`/`@refactor` по отдельной команде `/new-module` или прямому указанию пользователя.
- **Explain vs Create:** никогда не создавай карточку когда пользователь спрашивает «что это за задача / объясни / что значит» — это `intent: explain` → `@task-explain` (read-only). И наоборот: `intent: create` (`заведи/создай/новая задача`) → сразу `create.sh` без вопросов. Путаница — багуется классификатор, поправь `tools/classify_task.ts`.
